...
class sx126x:
    ...
    # cfg_reg = [0xC0,0x00,0x09,0x00,0x00,0x00,0x62,0x00,0x17,0x43,0x00,0x00] # Disabled: alternative config to save parameters permanently
    ...
    # power = 22       # Disabled: replaced by dynamic variables in __init__
    # air_speed =2400  # Disabled: replaced by dynamic variables in __init__
    ...
    def set(self,freq,addr,power,rssi,air_speed=2400,\
        ...
        # if air_speed_temp != None     # Unfinished: incomplete validation statement left by developer
        
        buffer_size_temp = self.lora_buffer_size_dic.get(buffer_size,None)
        # if air_speed_temp != None:    # Unfinished: incomplete validation statement left by developer
        
        power_temp = self.lora_power_dic.get(power,None)
        #if power_temp != None:        # Unfinished: incomplete validation statement left by developer
        ...
        for i in range(2):
            ...
            if self.ser.inWaiting() > 0:
                ...
                if r_buff[0] == 0xC1:
                    pass                        # Placeholder: code runs normally without printing debug logs
                    # print("parameters setting is :",end='')
                    # for i in self.cfg_reg:
                        # print(hex(i),end=' ')  # Disabled debug log: prevents terminal clutter during configuration
                    ...
                    pass                        # Placeholder: prevents code crash when skipping error message
                    #print("parameters setting fail :",r_buff) # Disabled debug log: turned off default failure output
                break
            else:
                ...
                if i == 1:
                    ...
                    # time.sleep(2)             # Disabled: avoiding unnecessary delays in execution loop
                    # print('\x1b[1A',end='\r') # Disabled: turned off terminal line clearing for this step
...
# "20,868,Hello World"
    def send(self,data):
        ...
        self.ser.write(data)
        # if self.rssi == True:       # Disabled: calling 'get_channel_rssi' here breaks or slows down continuous transmission
    ...
    def receive(self):
        if self.ser.inWaiting() > 0:
            ...
            # print the rssi
            if self.rssi:
                # print('\x1b[3A',end='\r') # Disabled: stops console cursor from moving up and overwriting previous logs
                print("the packet rssi value: -{0}dBm".format(256-r_buff[-1:][0]))
                self.get_channel_rssi()
            else:
                pass                        # Placeholder: code does nothing when RSSI printing is disabled
                #print('\x1b[2A',end='\r')  # Disabled: stops console cursor from moving up when RSSI is false
    ...
    def get_channel_rssi(self):
        ...
        if re_temp[0] == 0xC1 and re_temp[1] == 0x00 and re_temp[2] == 0x02:
            ...
            # print("the last receive packet rssi value: -{0}dBm".format(256-re_temp[4])) # Disabled: redundant log since packet RSSI is handled in receive()
        else:
            # pass                            # Disabled: overridden by the print statement below to show failure
            print("receive rssi value fail")
            # print("receive rssi value fail: ",re_temp) # Disabled debug log: hides the raw corrupted byte response on failure
